...
/**
 * read_input - reads a line of input from stdin without the newline character
...
ssize_t read_input(info_t *info)
{
	...
	if (len)
	{
		...
		check_chain_cont(info, buf, &j, i, len);
		while (j < len)
		{
			if (is_chain_deli(info, buf, &j))
				break;
			j++;
		}
 ...
 * Return: s, the line of input read from STDIN
 */
